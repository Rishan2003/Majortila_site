...
const scenes = [
  { name: 'IELTS on Computer', short: 'IELTS', kicker: 'AMBITION MEETS PREPARATION', first: 'THINK', last: 'BEYOND.', copy: 'The score is only the beginning. Build the skills and confidence to open your next door with IELTS at HEXA’S Majortila.', image: './images/computer.webp', alt: 'Students practising in the HEXA’S computer lab', caption: 'Real practice. Bigger possibilities.', tag: 'ON COMPUTER', note: 'Your next chapter starts here.' },
  { name: 'Spoken English', short: 'SPOKEN', kicker: 'YOUR IDEAS DESERVE TO BE HEARD', first: 'OWN YOUR', last: 'VOICE.', copy: 'From the first conversation to the moment you own the room. Make English feel natural, one confident sentence at a time.', image: './images/classroom-presentation.webp', alt: 'A student presenting in a HEXA’S English classroom', caption: 'Find your people. Find your voice.', tag: 'SPEAK WITH CONFIDENCE', note: 'Less hesitation. More you.' },
  { name: 'HICU Intensive', short: 'HICU', kicker: 'BIG GOALS. FOCUSED PREPARATION.', first: 'MAKE IT', last: 'HAPPEN.', copy: 'Bring your ambition. Find your focus. Turn practice, personal feedback and daily momentum into a stronger IELTS preparation journey.', image: './images/venue2.webp', alt: 'The HEXA’S IELTS on Computer test centre', caption: 'A clear goal. A stronger next step.', tag: 'INTENSIVE PREPARATION', note: 'Make every practice count.' },
]
const duration = 7200
  ...
  return <section ref={hero} className={`kinetic-hero scene-${active}`} id="home" aria-label="Explore learning at HEXA’S" aria-roledescription="carousel" data-hero-scroll style={{'--scene-duration':`${duration}ms`}} onFocus={()=>setFocused(true)} onBlur={e=>{if(!e.currentTarget.contains(e.relatedTarget))setFocused(false)}} onKeyDown={e=>{if(e.target.closest('.scene-navigation')){if(e.key==='ArrowRight'){e.preventDefault();choose(active+1)}if(e.key==='ArrowLeft'){e.preventDefault();choose(active-1)}}}}>
    ...
    <div className="container kinetic-hero-layout">
      <div className="kinetic-copy">
        ...
        <div key={`copy-${active}-${revision}`} className="scene-copy" aria-live="off"><div className="hero-kicker">{scene.kicker}</div><h1 aria-label={`${scene.first} ${scene.last}`}><LetterLine text={scene.first}/><LetterLine text={scene.last} accent/></h1><p>{scene.copy}</p></div>
        ...
        <div className="hero-social-proof"><div className="proof-faces" aria-hidden="true"><img src="./images/student4.webp" alt=""/><img src="./images/student2.webp" alt=""/><img src="./images/student0.webp" alt=""/></div><div><strong>Your ambition. Our commitment.</strong><span>A real community. A personal approach.</span></div></div>
      </div>

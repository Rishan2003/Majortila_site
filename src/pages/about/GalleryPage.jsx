...
export default function GalleryPage() {
  ...
  return <>
    <PageHero eyebrow="Life at HEXA’S" title="Small moments." accent="Big memories." description="আমাদের classroom আর test venue ঘুরে দেখুন। সাথে থাকছে students-দের সাফল্যের কিছু special moment।"/>
    <section className="content-section"><div className="container">
      ...
      {visible.length===0&&<div className="empty-state"><strong>No photos for this band yet.</strong><span>সব achievements দেখতে All bands বেছে নিন।</span></div>}
    </div></section>
    <dialog ref={dialog} className="lightbox" aria-label="Photo viewer" onClose={()=>setSelected(null)} onCancel={()=>setSelected(null)} onClick={e=>{if(e.target===e.currentTarget)setSelected(null)}} onKeyDown={e=>{if(e.key==='ArrowRight'){e.preventDefault();move(1)}if(e.key==='ArrowLeft'){e.preventDefault();move(-1)}}}>
      {photo&&<div className="lightbox-inner"><button className="lightbox-close" onClick={()=>setSelected(null)} aria-label="Close photo">×</button><img src={photo.image} alt={photo.name}/><div className="lightbox-bottom"><div aria-live="polite"><strong>{photo.name}</strong><p lang="bn">{photo.description} · {selected+1} / {visible.length}</p></div><div className="lightbox-controls"><button onClick={()=>move(-1)} aria-label="Previous photo"><Arrow/></button><button onClick={()=>move(1)} aria-label="Next photo"><Arrow/></button></div></div></div>}
    </dialog>
  </>
}
